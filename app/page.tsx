...
export default function LandingPage() {
  return (
          ...
          <div className="flex items-center gap-4">
            <SignedOut>
			  ...
			  <SignUpButton mode="modal">
                <button className="bg-brand hover:bg-brand-dark text-ink text-sm font-bold px-5 py-2.5 transition-colors shadow-sm">
                  Sign Up
                </button>
              </SignUpButton>
            </SignedOut>
            <SignedIn>
              <Link href="/dashboard">
                <button className="bg-brand hover:bg-brand-dark text-ink text-sm font-bold px-5 py-2.5 transition-colors shadow-sm flex items-center gap-2">
   ...
   {/* HERO SECTION */}
      <header className="pt-40 pb-0 relative overflow-hidden flex flex-col items-center text-center">
        
        {/* Text Wrapper (Maintains padding for text only) */}
        <div className="px-6 flex flex-col items-center w-full relative z-20">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-brand-light border border-brand/20 text-brand-dark text-[10px] font-bold uppercase tracking-[0.2em] mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <SignedOut>
              <SignUpButton mode="modal">
                <button className="w-full sm:w-auto bg-brand hover:bg-brand-dark text-ink font-bold px-8 py-4 text-sm rounded-full transition-colors shadow-sm flex items-center justify-center gap-2">
            ...
            <Link href="#curriculum" className="w-full sm:w-auto">
              <button className="w-full sm:w-auto bg-transparent border border-border-strong text-ink hover:bg-surface-muted rounded-full font-bold px-8 py-4 text-sm transition-colors">
                Explore Curriculum
              </button>
            </Link>
          </div>
        ...
        {/* Mountain Images (Full bleed, banner crop) */}
        <div className="w-full mt-12 pointer-events-none translate-y-1 flex justify-center z-10">
          <img 
            src="/hero-landscape.jpg" 
            alt="Himalayan Mountains" 
            /* h-[300px] to h-[400px] creates the banner shape. object-center trims top/bottom equally */
            className="hidden md:block w-full h-[300px] lg:h-[400px] object-cover object-center mix-blend-multiply" 
          />
          <img 
            src="/hero-mobile.png" 
            alt="Himalayan Mountains" 
            /* Keeps the mobile version from taking up the entire screen height */
            className="block md:hidden w-full h-[280px] object-cover object-center mix-blend-multiply" 
          />
        </div>
      </header>
	  ...
	  {/* FEATURES SECTION */}
      <section id="methodology" className="bg-ink text-white py-24">
      ...
      <section className="py-24 bg-surface text-center px-6">
        <div className="bg-ink max-w-4xl mx-auto p-12 md:p-16 text-center text-white relative overflow-hidden">
		  ...
		  <SignedOut>
            <SignUpButton mode="modal">
              <button className="bg-brand hover:bg-brand-dark text-ink font-bold px-8 py-4 text-sm transition-colors shadow-sm relative z-10">
          ...
          <SignedIn>
            <Link href="/dashboard" className="relative z-10">
              <button className="bg-brand hover:bg-brand-dark text-ink font-bold px-8 py-4 text-sm transition-colors shadow-sm">
                Open Dashboard
              </button>
            </Link>
          </SignedIn>
