...
import {
  ...
  Info, CheckCircle2, Moon, Sun, BookOpen, Anchor, Loader2, Shuffle, AlertTriangle
} from "lucide-react";
          ...
          <StepContainer index={0} step={STEPS[0]} status={statusOf(0)} isExpanded={expandedStep === 0} onToggle={() => toggleStep(0)} onContinue={() => markComplete(0)}>
            <div className="mb-6 flex flex-col border-b border-border-strong pb-4">
              <h2 className="font-serif text-2xl text-ink mb-1">
                How a Tibetan word is built <span className="font-serif italic text-ink-muted ml-2">ཚིག་གི་གྲུབ་སྟངས།</span>
              </h2>
            </div>
            <p className="mb-8 max-w-3xl text-[15px] leading-relaxed text-ink-light">
              A Tibetan syllable is built from up to <span className="font-bold text-ink">seven slots</span> arranged around a single <span className="font-bold text-ink">root letter</span>. Each slot has a name, a position, and a job. The <span className="font-bold text-brand-dark">prefix</span> is the leftmost of them.
            </p>
            ...
            <Card className="p-0 overflow-hidden mb-6">
              <div className="border-b border-border-strong bg-gradient-to-b from-stone-50 to-white px-6 pb-10 pt-8 text-center md:pb-14 relative group">
                ...
                <button 
                  ...
                  disabled={playingItem !== null}
                  className="text-tibetan-display transition-colors hover:text-brand-dark" 
                ...
                <div className="mt-6 text-[15px] italic text-ink-light">
                  <span className="not-italic font-bold text-ink">bsgrims</span> — “concentrated” · read <span className="not-italic font-bold text-ink">drim</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 border-t border-border-strong bg-surface-muted px-6 py-4 text-xs text-ink-light font-bold">
                <Info size={16} className="text-brand shrink-0" />
                <span>Not every syllable uses all seven slots — only the <span className="font-bold text-ink">root letter</span> is required.</span>
              </div>
            </Card>
          </StepContainer>
          ...
          <StepContainer index={1} step={STEPS[1]} status={statusOf(1)} isExpanded={expandedStep === 1} onToggle={() => toggleStep(1)} onContinue={() => markComplete(1)}>
            <div className="grid gap-4 md:grid-cols-3">
              <Card className="p-6 bg-surface">
                ...
                <p className="text-sm leading-relaxed text-ink-light">
                  A prefix is a letter written <span className="font-bold text-ink">to the left</span> of the root. Only five letters — <span className="font-serif text-lg">ག ད བ མ འ</span> — may take that seat.
                </p>
              </Card>
              <Card className="p-6 bg-surface">
                <div className="mb-3 inline-flex items-center gap-2 bg-brand-light px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-brand-dark">
                ...
                <p className="text-sm leading-relaxed text-ink-light">
                  Prefixes disambiguate words on the page — e.g. <span className="font-serif text-lg">བཞི་</span> vs <span className="font-serif text-lg">གཞི་</span>.
                </p>
              </Card>
              <Card className="p-6 bg-surface">
                <div className="mb-3 inline-flex items-center gap-2 bg-brand-light px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-brand-dark">
                ...
                <p className="text-sm leading-relaxed text-ink-light">
                  Prefixes never change <span className="font-bold text-ink">masculine</span> letters. They <span className="font-bold text-sky-700">deepen</span> feminine roots and <span className="font-bold text-rose-700">nasalise</span> very-feminine roots.
                </p>
              </Card>
            </div>
            ...
            <div className="mt-6 p-6 border border-border-strong bg-surface">
              <div className="mb-4 flex items-center gap-2 text-eyebrow">
                <Info size={14} className="text-brand" /> Two rules of writing
              </div>
              <ol className="space-y-4 text-[15px] text-ink-light font-bold">
                <li className="flex gap-4">
                  <span className="grid size-6 shrink-0 place-items-center rounded bg-surface-muted border border-border-strong text-[11px] text-ink-light">1</span>
                  <span>Only five letters — <span className="font-serif text-xl text-ink mx-1 px-2 py-0.5 bg-surface-muted border border-border-strong">{NEVER_TAKE}</span> — <em>never</em> take a prefix.</span>
                </li>
                <li className="flex gap-4">
                  <span className="grid size-6 shrink-0 place-items-center rounded bg-surface-muted border border-border-strong text-[11px] text-ink-light">2</span>
                  <span>A root that follows a prefix must carry <em>at least</em> a vowel, superscript, or subscript.</span>
                </li>
              </ol>
            </div>
          </StepContainer>
    ...
    <div className={`relative overflow-hidden border transition-colors duration-500 ${night ? "border-white/10 bg-[#0f0d0a] text-stone-100" : "border-border-strong bg-surface"}`}>
      ...
      <div className="grid gap-6 p-6 md:grid-cols-[auto,1fr] md:p-8 border-b border-border-strong">
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className={`text-eyebrow mb-2 ${night ? "text-stone-400" : ""}`}>
          ...
          {p.combos.slice(0, 6).map((c: any) => {
            ...
            return (
              <div key={c.word + c.read} className={`flex flex-wrap items-center gap-x-6 gap-y-3 border px-5 py-4 ${night ? "border-white/10 bg-white/5" : "border-border-strong bg-surface shadow-sm"}`}>
                ...
                {c.gloss && (
                  <span className={`text-[13px] font-bold italic ${night ? "text-stone-400" : "text-ink-light"}`}>
                    {c.gloss}
                  </span>
                )}
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        ...
        {question.answer.gloss && (
          <span className={`italic font-bold ${night ? "text-stone-400" : "text-ink-light"}`}>“{question.answer.gloss}”</span>
        )}
  ...
  return (
    <>
      ...
      <div className="mt-10 border border-border-strong bg-surface-muted p-6 flex flex-col items-center sm:items-start sm:flex-row gap-6">
        <div className="bg-surface border border-border-strong p-3 shadow-sm shrink-0">
